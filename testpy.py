...
syntax_default = ctypes.byref(onig.OnigSyntaxType())
onig.onig_copy_syntax(syntax_default, onig.ONIG_SYNTAX_DEFAULT)
onig.onig_set_syntax_options(syntax_default,
        onig.onig_get_syntax_options(syntax_default)
            & ~onig.ONIG_OPTION_ASCII_RANGE)


def get_encoding_name(onigenc):
    """Return the name of specified onig.OnigEncoding.

    arguments:
      enc -- an instance of onig.OnigEncoding
    """
...
def is_unicode_encoding(enc):
    ...
    arguments:
      enc -- encoding name or an instance of onig.OnigEncoding
    """
...
def xx(pattern, target, s_from, s_to, mem, not_match,
        ...
        err=onig.ONIG_NORMAL, execerr=onig.ONIG_NORMAL):
    global nerror
    global nsucc
    global nfail
    ...
    msg = ctypes.create_string_buffer(onig.ONIG_MAX_ERROR_MESSAGE_LEN)
    ...
    # Compile
    r = onig.onig_new(ctypes.byref(reg),
            patternp.getptr(), patternp.getptr(-1),
            opt, onig_encoding, syn, ctypes.byref(einfo));
    if r != 0:
        # Error
        onig.onig_error_code_to_str(msg, r, ctypes.byref(einfo))
        if r == err:
            nsucc += 1
    ...
    if searchtype == SearchType.FORWARD:
        ...
        r = onig.onig_match(reg, targetp.getptr(), targetp.getptr(-1),
...
def set_default_warning_function():
    ...
    _warn_func_ptr = onig.OnigWarnFunc(warn_func)
    onig.onig_set_warn_func(_warn_func_ptr)
    onig.onig_set_verb_warn_func(_warn_func_ptr)
...
#    x2(b"\\xca\\xb8", b"\xca\xb8", 0, 2);   # "文"
    ...
    n("[0-9-a]", "", syn=onig.ONIG_SYNTAX_GREP, err=onig.ONIGERR_UNMATCHED_RANGE_SPECIFIER_IN_CHAR_CLASS)
    ...
    #n("()" * 32768, "", err=onig.ONIGERR_TOO_MANY_CAPTURE_GROUPS)  # error code is different with Onigmo and Oniguruma
    ...
    #n("(\\2)(\\1)", "")     # Issue #65
    #n("(0?0|(?(1)||)|(?(1)||))?", "", err=onig.ONIGERR_INVALID_CONDITION_PATTERN) # Ruby Bug#12418
    #n("[\\40000000000", "", err=onig.ONIGERR_TOO_BIG_NUMBER)  # Ruby Bug#12420
    #n("[\\600000000000\n", "", err=onig.ONIGERR_TOO_BIG_NUMBER)   # Ruby Bug#12423
    n("[]", "", err=onig.ONIGERR_EMPTY_CHAR_CLASS)
    n("[c-a]", "", err=onig.ONIGERR_EMPTY_RANGE_IN_CHAR_CLASS)
    x2("[[:ab:\\x{30}]]+", ":ab0x", 0, 4)
    x2("[[:x\\]:]+", "[x:]", 0, 4)
    x2("[!--x]+", "!-x", 0, 3)
    x2(" ]", " ]", 0, 2)    # warning: ']' without escape
    n("\\x{FFFFFFFF}", "", err=onig.ONIGERR_TOO_BIG_WIDE_CHAR_VALUE);
    n("\\x{100000000}", "", err=onig.ONIGERR_TOO_LONG_WIDE_CHAR_VALUE);
    #x2("\\u0026", "\u0026", 0, 1)
    #x2("[\\u0024-\\u0027]", "\u0026", 0, 1)
    #n("\\u026x", "", err=onig.ONIGERR_TOO_SHORT_DIGITS)
    n("()(?\\!(?'a')\\1)", "", err=onig.ONIGERR_UNDEFINED_GROUP_OPTION)
    x2("\\i", "i", 0, 1)    # unknown escape warning
    n("\\((", "", err=onig.ONIGERR_END_PATTERN_WITH_UNMATCHED_PARENTHESIS)
    n("(|", "", err=onig.ONIGERR_END_PATTERN_WITH_UNMATCHED_PARENTHESIS)
    x2("%{(.*?)}", "%{HOSTNAME}", 0, 11)
    #if not is_ascii_incompatible_encoding(onig_encoding):
    #    n(b"'/g\\\xff\xff\xff\xff&))", "", err=onig.ONIGERR_UNMATCHED_CLOSE_PARENTHESIS)
    #    n(b"\\\xff0", "")
    if onig_encoding == onig.ONIG_ENCODING_UTF8:
        n(b"[0-0-\xe2  ", "", err=onig.ONIGERR_PREMATURE_END_OF_CHAR_CLASS)
    n("\\p{foobarbaz}", "", err=onig.ONIGERR_INVALID_CHAR_PROPERTY_NAME)
    n("\\p{あ}", "", err=onig.ONIGERR_INVALID_CHAR_PROPERTY_NAME)
    if is_unicode_encoding(onig_encoding):
        n("\\p{\U00025771}", "", err=onig.ONIGERR_INVALID_CHAR_PROPERTY_NAME)
    if onig_encoding == onig.ONIG_ENCODING_UTF8:
        x2("[\\xce\\xb1\\xce\\xb2]", "β", 0, 1)
    elif onig_encoding == onig.ONIG_ENCODING_SJIS:
        n("[\\x84A]", "", err=onig.ONIGERR_TOO_SHORT_MULTI_BYTE_STRING)
    elif onig_encoding == onig.ONIG_ENCODING_EUC_JP:
        n("[\\xAAA]", "", err=onig.ONIGERR_TOO_SHORT_MULTI_BYTE_STRING)
    elif is_ascii_incompatible_encoding(onig_encoding):
        n("[\\x420]", "", err=onig.ONIGERR_TOO_SHORT_MULTI_BYTE_STRING)
    ...
    n("a{100001}", "", err=onig.ONIGERR_TOO_BIG_NUMBER_FOR_REPEAT_RANGE)
    n("a{0,100001}", "", err=onig.ONIGERR_TOO_BIG_NUMBER_FOR_REPEAT_RANGE)
    n("a{5,1}", "", err=onig.ONIGERR_UPPER_SMALLER_THAN_LOWER_IN_REPEAT_RANGE)
    x2("abc{1}", "abcc", 0, 3)
    ...
    x2("a*", "aa aaa aaaa aaaaa ", 12, 17, opt=onig.ONIG_OPTION_FIND_LONGEST)

    # ONIG_OPTION_FIND_NOT_EMPTY option
    x2("\w*", " a", 0, 0)
    x2("\w*", " a", 1, 2, opt=onig.ONIG_OPTION_FIND_NOT_EMPTY)

    # ONIG_OPTION_DONT_CAPTURE_GROUP option
    x2("(ab|cd)*", "cdab", 0, 4, opt=onig.ONIG_OPTION_DONT_CAPTURE_GROUP)
    n("(ab|cd)*\\1", "", opt=onig.ONIG_OPTION_DONT_CAPTURE_GROUP, err=onig.ONIGERR_INVALID_BACKREF)
...
#    x2("a\\K?a", "aa", 0, 2)        # error: differ from perl
    ...
    #x2("((?<v>)a)|b(?0)b", "bbabb", 0, 5, syn=onig.ONIG_SYNTAX_PERL)
    #x2("((?<v>)a|b(?1)b)", "bbabb", 0, 5, syn=onig.ONIG_SYNTAX_PERL)
    #x2("((?<v>a|b(?&v)b))", "bbabb", 0, 5, syn=onig.ONIG_SYNTAX_PERL)
    #n("(?<", "", err=onig.ONIGERR_END_PATTERN_WITH_UNMATCHED_PARENTHESIS)
    n("(?<>)", "", err=onig.ONIGERR_EMPTY_GROUP_NAME)
    n("(?<.>)", "", err=onig.ONIGERR_INVALID_CHAR_IN_GROUP_NAME)
    #n("\\g<1->", "", err=onig.ONIGERR_INVALID_CHAR_IN_GROUP_NAME)
    n("\\k<1/>", "", err=onig.ONIGERR_INVALID_GROUP_NAME)
    n("\\k<1-1/>", "", err=onig.ONIGERR_INVALID_GROUP_NAME)
    n("\\k<a/>", "", err=onig.ONIGERR_INVALID_CHAR_IN_GROUP_NAME)
    ...
    # Grep syntax
    # \+, \?, \|, \{n,m\}
    x2("a\\+", "aa", 0, 2, syn=onig.ONIG_SYNTAX_GREP)
    n("a\\+", "b", syn=onig.ONIG_SYNTAX_GREP)
    x2("a\\?", "", 0, 0, syn=onig.ONIG_SYNTAX_GREP)
    x2("a\\?", "a", 0, 1, syn=onig.ONIG_SYNTAX_GREP)
    x2("ab\\|cd", "cd", 0, 2, syn=onig.ONIG_SYNTAX_GREP)
    x2("a\\{1,2\\}", "aaa", 0, 2, syn=onig.ONIG_SYNTAX_GREP)
    x2("a\\{2\\}", "aaa", 0, 2, syn=onig.ONIG_SYNTAX_GREP)
    n("a\\{|", "", syn=onig.ONIG_SYNTAX_GREP, err=onig.ONIGERR_END_PATTERN_AT_LEFT_BRACE)
    # \< and \>
    ...
    # others
    n("[^a]", "\n", syn=onig.ONIG_SYNTAX_GREP)
    x2("*", "*", 0, 1, syn=onig.ONIG_SYNTAX_GREP)
    #x2("\\{1\\}", "{1}", 0, 3, syn.onig.ONIG_SYNTAX_GREP)    # fails
    n("*", "", err=onig.ONIGERR_TARGET_OF_REPEAT_OPERATOR_NOT_SPECIFIED)
    n("{1}", "", err=onig.ONIGERR_TARGET_OF_REPEAT_OPERATOR_NOT_SPECIFIED)

    ## \g{} backref
    #x2("((?<name1>\\d)|(?<name2>\\w))(\\g{name1}|\\g{name2})", "ff", 0, 2, syn=onig.ONIG_SYNTAX_PERL);
    #x2("(?:(?<x>)|(?<x>efg))\\g{x}", "", 0, 0, syn=onig.ONIG_SYNTAX_PERL);
    #x2("(?:(?<x>abc)|(?<x>efg))\\g{x}", "efgabcabc", 3, 9, syn=onig.ONIG_SYNTAX_PERL);
    ...
    #n("(?P", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_UNDEFINED_GROUP_OPTION)
    #n("(?PX", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_UNDEFINED_GROUP_OPTION)
...
#    x3("(?|(c)|(?|(b)|(a)))", "a", 0, 1, 1)
    ...
    #n("()(?(2))", "", err=onig.ONIGERR_INVALID_BACKREF)       # Issue #65
    #n("(?(700000))", "", err=onig.ONIGERR_INVALID_BACKREF)
    ...
    # Perl syntax
    x2("\\Q()\\[a]\\E[b]", "()\\[a]b", 0, 7, syn=onig.ONIG_SYNTAX_PERL)
    x2("\\Q()\\[a]", "()\\[a]", 0, 6, syn=onig.ONIG_SYNTAX_PERL)  # no \E
    #x2("(?a)(?d)\\w+", "あ", 0, 1, syn=onig.ONIG_SYNTAX_PERL) # For now (?d) == (?u)
    #x2("(?a)(?l)\\w+", "あ", 0, 1, syn=onig.ONIG_SYNTAX_PERL) # For now (?l) == (?u)
    #x2("(?a)(?^)\\w+", "あ", 0, 1, syn=onig.ONIG_SYNTAX_PERL)
    #n("(?i)(?^)a", "A", syn=onig.ONIG_SYNTAX_PERL)
    #n("(?m)(?^)a$", "a\nb", syn=onig.ONIG_SYNTAX_PERL)
    #x2("(?s)(?^).*", "a\nb", 0, 1, syn=onig.ONIG_SYNTAX_PERL)
    #x2("\\o{046}", "\046", 0, 1, syn=onig.ONIG_SYNTAX_PERL);
    #x2("[\\o{044}-\\o{047}]", "\046", 0, 1, syn=onig.ONIG_SYNTAX_PERL);
    #n("\\o{40000000000}", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_TOO_BIG_WIDE_CHAR_VALUE);
    #n("\\o{100000000000}", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_TOO_LONG_WIDE_CHAR_VALUE);
    #n("[\\o{40000000000}]", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_TOO_BIG_WIDE_CHAR_VALUE);
    #n("[\\o{100000000000}]", "", syn=onig.ONIG_SYNTAX_PERL, err=onig.ONIGERR_TOO_LONG_WIDE_CHAR_VALUE);
    ...
    x2(".*[a-z]bc", "abcabc", 3, 6, searchtype=SearchType.BACKWARD)
    x2(".+[a-z]bc", "abcabc", 2, 6, searchtype=SearchType.BACKWARD)
    ...
    # stack size
    stack_size = onig.onig_get_match_stack_limit_size()
    print("Default stack size:", stack_size)
    onig.onig_set_match_stack_limit_size(1000)
    print("New stack size:", onig.onig_get_match_stack_limit_size())
    # These patterns need deep stack.
    n("^a*$", "a" * 200 + "b")
    n("^a*$", "a" * 2000 + "b", execerr=onig.ONIGERR_MATCH_STACK_LIMIT_OVER)
    onig.onig_set_match_stack_limit_size(0)

    ## parse depth
    #parse_depth = onig.onig_get_parse_depth_limit()
    #print("Default parse depth:", parse_depth)
    #onig.onig_set_parse_depth_limit(1000)
    #print("New parse depth:", onig.onig_get_parse_depth_limit())
    ## These patterns need deep parse stack.
    #x2("(" * 200 + "a" + ")" * 200, "a", 0, 1)
    #n("(" * 2000 + "a" + ")" * 2000, "a", err=onig.ONIGERR_PARSE_DEPTH_LIMIT_OVER)
    #onig.onig_set_match_stack_limit_size(0)

    # syntax functions
    onig.onig_set_syntax_op(syntax_default,
        onig.onig_get_syntax_op(onig.ONIG_SYNTAX_DEFAULT))
    onig.onig_set_syntax_op2(syntax_default,
        onig.onig_get_syntax_op2(onig.ONIG_SYNTAX_DEFAULT))
    onig.onig_set_syntax_behavior(syntax_default,
        onig.onig_get_syntax_behavior(onig.ONIG_SYNTAX_DEFAULT))
    onig.onig_set_default_syntax(None)


    print("\nEncoding:", get_encoding_name(onig_encoding))
    print("RESULT   SUCC: %d,  FAIL: %d,  ERROR: %d      (by Oniguruma/Onigmo %s)" % (
